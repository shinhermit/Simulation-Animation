...
{
public:
    ParticleSimulator(int debug=0, saViewer *viewer=NULL,
...
public slots:
    ...
    /// \brief Définit la distance maximale d'influence entre les particules de la simulation
    void setSmoothingTolerance(const float & coeff_d) throw(std::invalid_argument);

    /// \brief Définit le coefficient de dépression
    void setPressureToDensityGradientProportionnality(const float & coeff_k);

    /// \brief Définit le coefficient de viscosité dynamique
    void setDynamicViscosityConstant(const float & coeff_mu);

    /// \brief Définit le coefficient de viscosité dynamique
    void setInitialDensity(const float & coeff_rho0) throw(std::invalid_argument);
